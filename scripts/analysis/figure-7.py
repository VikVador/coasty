r"""Figure 7: Saturation percentage (O2_measure / O2_solubility * 100).
...
if __name__ == "__main__":
    ...
    saturation_pct = compute_saturation_percentage(ds, depth="bottom")
    
    valid = np.isfinite(saturation_pct)
    print(f"  Profiles with valid saturation %: {valid.sum():,} / {len(saturation_pct):,}")
    
    out_dir = Path(__file__).parent.parent.parent / "plots" / "analysis" / "figure-7"
    ...
    fig.savefig(
        out_dir / "figure-7-saturation-percentage-bottom.pdf",
        dpi=FIGURE_DPI_SAVE,
        bbox_inches="tight",
    )
    last_fig = fig
    ...
    for decade in decades:
        ...
        fig.savefig(
            out_dir / f"figure-7-saturation-percentage-{decade}.pdf",
            dpi=FIGURE_DPI_SAVE,
            bbox_inches="tight",
        )
        last_fig = fig
